...
public:
	static NodeData* Alloc(void){ return NewObjClear(RevolvedMesh); }
...
private:
	...
	SplineLengthData* _childSplineLD;		///< Retains the pointer to the SplineLengthData helper instance of the revolved profile.
	...
	BaseObject*				_lastChild;				///< Retains the pointer to the BaseObject instance of the generator's child.
};
...
BaseObject* RevolvedMesh::GetVirtualObjects(BaseObject* op, HierarchyHelp* hh)
{
	...
	const Bool childIsDirty = child->IsDirty(DIRTYFLAGS_ALL);
	// Check the dirty status of the generator container.
	const Bool opDataIsDirty = op->IsDirty(DIRTYFLAGS_DATA);
	...
	if (!RevolvedMeshHelpers::ResizeverticesBaseArray(verticesBA, stepsS + 1, stepsT + 1, crvClosed, isFullSpin))
	{
		SplineLengthData::Free(_childSplineLD);
		return BaseObject::Alloc(Onull);
	}

	if (!FillverticesPosition(verticesBA, stepsS, stepsT, spinningVector, spinningStartAngle, spinningEndAngle, crvClosed, isFullSpin))
	{
		SplineLengthData::Free(_childSplineLD);
		return BaseObject::Alloc(Onull);
	}
	...
	if (RevolvedMeshHelpers::FillPolygonObjectData(*polyObj, stepsS + 1, stepsT + 1, hh, verticesBA, crvClosed, isFullSpin))
...
Bool RegisterRevolvedMesh()
{
	...
	return RegisterObjectPlugin(ID_SDKEXAMPLE_OBJECTDATA_REVOLVEDMESH, registeredName, OBJECT_GENERATOR | OBJECT_INPUT, RevolvedMesh::Alloc, "orevolvedmesh", AutoBitmap("revolvedmesh.tif"), 0);
}